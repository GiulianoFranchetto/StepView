...
public class StepView extends View {
	...
	private void applyStyles(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		errorCircleColor = ta.getColor(R.styleable.StepView_errorCircleColor, 0);
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < startLinesX.length; i++) {
			if (state == ANIMATE_STEP_TRANSITION && i == nextAnimatedStep - 1
					&& nextAnimatedStep > currentStep && (animationType == ANIMATION_LINE || animationType ==
					ANIMATION_ALL)) {
	...
	private void drawStep(Canvas canvas, int step, int circleCenterX, int circleCenterY) {
		...
		final boolean isError = error ? step <= currentStep : step < currentStep;
		final String number = String.valueOf(step + 1);

		if (isSelected && !isDone && !isError) {
			paint.setColor(selectedCircleColor);
			int radius;
			if (state == ANIMATE_STEP_TRANSITION && (animationType == ANIMATION_CIRCLE || animationType ==
					ANIMATION_ALL)
			...
			drawErrorMark(canvas, circleCenterX, circleCenterY);
	...
	private void drawErrorMark(Canvas canvas, int circleCenterX, int circleCenterY) {
		paint.setColor(doneStepMarkColor);
		float width = stepNumberTextSize * 0.1f;
		paint.setStrokeWidth(width);
		Rect bounds = new Rect(
